...
export function ReplyPreview({ replyTo, onCancel, compact = false }: ReplyPreviewProps) {
    return (
        <div
            className={`flex items-start gap-2 ${compact ? 'px-3 py-2 bg-[var(--bg-surface)] rounded-lg' : 'px-4 py-2.5 bg-[var(--accent-light)] border-l-2 border-[var(--accent)] rounded-r-lg'}`}
        >

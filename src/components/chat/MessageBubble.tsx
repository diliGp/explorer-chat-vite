...
export function MessageBubble({
    ...
    if (message.deletedAt && message.type === 'text') {
        return (
            <div className={`flex ${isSelf ? 'justify-end' : 'justify-start'} px-4 py-0.5`}>
                <div className="text-xs text-[var(--text-muted)] italic px-2">Message deleted</div>
            </div>
        );
    }
    ...
    return (
        ...
        >
            <div
                className={`max-w-[75%] flex flex-col ${isSelf ? 'items-end' : 'items-start'} gap-1`}
            >
                {/* Sender name (non-self only) */}
                {!isSelf && (
                    <span className="text-xs font-medium text-[var(--accent)] px-1">
                        {message.senderName}
                    </span>
                )}
                ...
                >
                    ...
                    {message.type === 'image' && (
                        <ImageMessage message={message} isSelf={isSelf} onView={onViewImage} />
                    )}
                </div>

                {/* Timestamp + actions */}
                <div
                    className={`flex items-center gap-2 px-1 ${isSelf ? 'flex-row-reverse' : 'flex-row'}`}
                >

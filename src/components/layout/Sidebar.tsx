...
export function Sidebar() {
    ...
    const recentOffline = recentChats
        .filter(
            (rc) => !onlineOtherUids.has(rc.otherUser.uid) && rc.otherUser.uid !== currentUser?.uid
        )
    ...
    return (
        ...
        >
            ...
            <div className="hidden sm:flex items-center justify-between px-4 py-4 border-b border-[var(--border)]">
                <Link
                    to="/"
                    className="flex items-center gap-2 font-bold text-[var(--text-primary)] text-lg"
                >
            ...
            <div className="px-4 py-3 border-b border-[var(--border)]">
                <p className="text-xs font-semibold text-[var(--text-muted)] uppercase tracking-wider flex items-center gap-2">
                    ...
                    {loading
                        ? '...'
                        : `${onlineUsers.filter((u) => u.uid !== currentUser?.uid).length} Online`}
                </p>
            </div>
            ...
            {/* Users list */}
            <div className="flex-1 overflow-y-auto px-2 py-2" aria-label="Users">
                {loading ? (
                    <div className="flex flex-col gap-2 px-2">
                        {Array.from({ length: 5 }).map((_, i) => (
                            <div
                                key={i}
                                className="h-14 bg-[var(--bg-surface)] rounded-xl animate-pulse"
                            />
                        ))}
                    </div>
                ) : (

 ...
 */
async function mergeAnonymousData(anonUid: string, permanentUid: string): Promise<void> {
    ...
    for (const dmDoc of dmsSnap.docs) {
        ...
        for (const [uid, country] of Object.entries(dm.participantCountries ?? {})) {
        ...
        const msgsSnap = await getDocs(collection(db, 'dms', oldDmId, 'messages'));
